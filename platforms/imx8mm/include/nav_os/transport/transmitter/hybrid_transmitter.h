...
void HybridTransmitter<M>::Enable() {
  ...
  // transmitters_[OptionalMode::INTRA]->Enable();
}
...
void HybridTransmitter<M>::Enable(const RoleAttributes& opposite_attr) {
  ...
  //SPDLOG_INFO("Debug HybridTransmitter<M>::Enable self_id =[{}] ",
 //        this->id_.ToString());
...
template <typename M>
bool HybridTransmitter<M>::Transmit(const MessageTypePtr& msg,
                                    const MessageInfo& msg_info) {
  //SPDLOG_INFO("DEBUG HybridTransmitter<M>::Transmit");
  std::lock_guard<std::mutex> lock(mutex_);
  for (auto& item : transmitters_) {
    // 4 // 6
  ...
  for (auto& mode : modes) {
    switch (mode) {
      case OptionalMode::INTRA:
        transmitters_[mode] =
            std::make_shared<IntraTransmitter<M>>(this->role_attr_);
        break;
      case OptionalMode::SHM:
        transmitters_[mode] = std::make_shared<ShmTransmitter<M>>(this->role_attr_);
        break;
      default:
        SPDLOG_WARN("OptionNode is {:d}", mode);
        break;
    }
  }
}
