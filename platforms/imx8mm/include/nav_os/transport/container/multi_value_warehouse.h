...
#include "transport/container/warehouse_base.h"
...
* @brief[in] 保存reader/writer的role_attributes属性的容器, 上层接口wrtier::tranmit()能够自适应的去判断通信链路，
* 是因为底层的通道能够对通道两边的reader和wrtier之间的role_attributes对比后作出判断判断。
* 因此一个通道的底层是需要维持通道两边的reader/writer的role_attributes, 而multi_map是一种
* 可以允许key值重复, value值不同的map, 刚好适用于通道id与readers/writers一对多的关系
*/
