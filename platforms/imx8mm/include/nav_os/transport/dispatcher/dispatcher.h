...
class Dispatcher {
 public:
  Dispatcher();
  virtual ~Dispatcher();
  virtual void Init() { is_shutdown_.store(false); }
...
void Dispatcher::AddListener(const RoleAttributes& self_attr,
  ...
  if (msg_listeners_.Get(channel_id, &handler_base)) {
    ...
    if (handler == nullptr) {
      SPDLOG_ERROR("please ensure that readers with the same channel[{}] in the same process, \
        have the same message type",self_attr.channel_name());
      return;
    }
  } else {
    SPDLOG_DEBUG("new reader for channel: {}",GlobalData::GetChannelById(channel_id));
...
void Dispatcher::AddListener(const RoleAttributes& self_attr,
  ...
  if (msg_listeners_.Get(channel_id, &handler_base)) {
    handler =
        std::dynamic_pointer_cast<ListenerHandler<MessageT>>(*handler_base);
    if (handler == nullptr) {
      SPDLOG_ERROR("please ensure that readers with the same channel[{}] in the same process have the same message type",self_attr.channel_name());
      return;
    }
  } else {
    SPDLOG_DEBUG("new reader for channel:",GlobalData::GetChannelById(channel_id));
